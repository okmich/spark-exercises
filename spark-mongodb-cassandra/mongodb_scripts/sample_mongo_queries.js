...
db.test_result.aggregate([
    {$match : {$and : [{'outcome.result_code' : {$in: ['P', 'F', 'PRS' ]}}, {$and : [{'test_date' : {$gt : new ISODate('2005-01-01')}}, {'test_date' : {$lte : new ISODate('2005-08-31')}}]}, {'test_type.type_code' : 'NT'}]}},
...
// -- RfR Volumes and Distinct Test Failures 2010 for Class 7 Vehicles by Top Level est Item Group (For vehicles as presented for initial test)
// -- ==================
db.test_result.aggregate([
    {$match : {$and : [{$and : [{'test_date' : {$gt : new ISODate('2005-01-01')}}, {'test_date' : {$lte : new ISODate('2005-12-31')}}]}, {'test_class_id' : 7}, {'outcome.result_code' : {$in: ['F', 'PRS' ]}}]}},
    {$project : {'test_id': 1, 'test_items': 1, '_id': 0}},
    {$unwind : '$test_items'},
    {$match : {'test_items.rfr_type_code' : {$in : ['F', 'P']}}},
    {$project : {'test_id': 1, 'item_name' : '$test_items.test_detail.item_group_level_1'}},
    {$group : {
            _id : '$item_name',
            'rfr_volume' : {$sum : 1},
