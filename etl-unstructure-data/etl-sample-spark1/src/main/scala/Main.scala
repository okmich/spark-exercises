...
object Main { 
    ...
    def main(args : Array[String]) : Unit = {
        ...
        //read the dataset
        val rawFileRDD = sparkContext.textFile("/user/cloudera/rawdata").
        ...
        val fileHeaderDF = parseAndStructureTextRDD(sqlContext, fileHeaderRDD, fileHeaderRecordDesc)
        val poolDF = parseAndStructureTextRDD(sqlContext, poolRDD, poolRecordDesc)
        val loanDF = parseAndStructureTextRDD(sqlContext, loanRDD, loanRecordDesc)
        val poolActiveDF = parseAndStructureTextRDD(sqlContext, poolActiveRDD, poolActiveRecordDesc)
        val fileTrailerDF = parseAndStructureTextRDD(sqlContext, fileTrailerRDD, fileTrailerRecordDesc)

        //save each of them in a file table or destination
        fileHeaderDF.write.insertInto("ginnie.file_header")
        poolDF.repartition(1).write.mode(SaveMode.Append).orc("/apps/hive/warehouse/ginnie.db/pool")
        loanDF.coalesce(2).write.mode(SaveMode.Append).orc("ginnie.loan")
        poolActiveDF.write.insertInto("ginnie.active_pool_summary")
        fileTrailerDF.write.insertInto("ginnie.file_trailer")
    }

    
    def parseAndStructureTextRDD(sqlContext: SQLContext, textRDD : RDD[String], 
        ...
        sqlContext.createDataFrame(rddRow, structType)
    }
